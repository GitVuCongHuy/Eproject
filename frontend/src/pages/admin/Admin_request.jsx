...
import { useAuth } from '../../context/Context'; // Đảm bảo đúng đường dẫn
...
const AdminRequest = () => {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      if (data.status === 200) {
        ...
        setError(data.message || 'Không thể tải yêu cầu.');
      }
    } catch (err) {
  ...
  const handleApprove = async (requestId) => {
    const confirm = window.confirm('Bạn có chắc muốn chấp nhận yêu cầu này?');
    if (!confirm) return;
    try {
      ...
      setError('Lỗi khi duyệt yêu cầu: ' + err.message);
    }
  };
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell><strong>Địa Chỉ Nhận</strong></TableCell>
              ...
              {loading ? (
                ...
                requests.map((req) => (
                  <TableRow key={req.requestId} hover>
                    ...
                    <TableCell>{req.email || 'Không rõ'}</TableCell>
                    <TableCell>{req.customerName || 'Ẩn danh'}</TableCell>
                    <TableCell>{formatDate(req.requestDate)}</TableCell>
                    <TableCell>{req.detail?.purpose || req.reason || 'Không có'}</TableCell>
                    <TableCell>{req.detail?.quantity || 'N/A'}</TableCell>
                    <TableCell>{req.detail?.deliveryAddress || 'N/A'}</TableCell>
                    <TableCell>
                      <Chip
                        label={req.status === 'Pending' ? 'Đang chờ' : req.status === 'Approved' ? 'Đã duyệt' : 'Đã từ chối'}
