...
const TransactionPasswordSettings = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const action = mode === 'change' ? 'đổi' : 'tạo';
      setSuccess(`${action.charAt(0).toUpperCase() + action.slice(1)} mật khẩu giao dịch thành công!`);
  ...
  const PasswordField = ({ label, field, placeholder, required = true }) => (
    <TextField
      fullWidth
      label={label}
