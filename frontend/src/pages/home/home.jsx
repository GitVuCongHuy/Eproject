...
// <<< THÊM MỚI 1: Import các component cần thiết cho Dialog >>>
import { Box, Typography, Card, CardContent, Button, Avatar, useTheme, CircularProgress, Alert, GlobalStyles, IconButton, Tooltip, Chip, Fade, Paper, Divider, Stack, Dialog, DialogTitle, DialogContent, DialogActions } from '@mui/material';
// <<< THÊM MỚI 2: Import thêm các icon cho Dialog >>>
import { CreditCard, ArrowUpward, ArrowDownward, ChevronRight, Clear, PictureAsPdf, TrendingUpRounded, AccountBalanceWallet, AttachMoney, FilterList, Visibility, VisibilityOff, ReceiptLong, CalendarToday, Notes, AccountCircle, Send, CallReceived } from '@mui/icons-material';
...
const Home = () => {
  ...
  // <<< THÊM MỚI 3: State để quản lý Dialog chi tiết giao dịch >>>
  const [isDetailModalOpen, setIsDetailModalOpen] = useState(false);
  const [selectedTransaction, setSelectedTransaction] = useState(null);

  // <<< THÊM MỚI 4: Hàm để mở và đóng Dialog >>>
  ...
  const handleCloseDetailModal = () => {
    ...
    // Xóa dữ liệu giao dịch đã chọn sau khi đóng để tránh hiển thị nhầm
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try { setLoading(true); setError(null); const userResult = await getUserInfo(); if (userResult.success) setUser(userResult.data); else throw new Error(userResult.message || 'Không thể lấy thông tin người dùng.'); const cardsResult = await getCards(); if (cardsResult.success) { setCards(cardsResult.data); const initialVisibility = cardsResult.data.reduce((acc, card) => { acc[card.account_id] = false; return acc; }, {}); setCardBalanceVisibility(initialVisibility); if (cardsResult.data && cardsResult.data.length > 0) { setSelectedCardId(cardsResult.data[0].account_id); } } else { throw new Error(cardsResult.message || 'Không thể lấy danh sách thẻ.'); } } catch (err) { setError(err.message); } finally { setLoading(false); }
    };
    fetchInitialData();
  ...
  useEffect(() => {
    ...
    const fetchTransactionsForCard = async () => {
      setIsFetchingTransactions(true); setTransactions([]); setError(null);
      try {
        const now = new Date(); const month = now.getMonth() + 1; const year = now.getFullYear(); const transactionsResult = await getTransactions(selectedCardId, month, year);
        if (transactionsResult.success) {
          const selectedCard = cards.find(card => card.account_id === selectedCardId); if (!selectedCard) { throw new Error("Không tìm thấy thông tin thẻ đã chọn."); }
          ...
          const processedTransactions = transactionsResult.data.map(t => {
            const senderAccountNumber = t.senderAccount?.replace(/\s/g, ''); const isOutgoing = senderAccountNumber === currentCardNumber;
            return { ...t, transaction_type: isOutgoing ? 'Withdrawal' : 'Deposit', };
          });
          const sortedTransactions = processedTransactions.sort((a, b) => new Date(b.transactionDate) - new Date(a.transactionDate)); setTransactions(sortedTransactions);
        } else { setError(`Không thể lấy lịch sử giao dịch cho thẻ này. ${transactionsResult.message || ''}`); }
      } catch (err) { setError(err.message); } finally { setIsFetchingTransactions(false); }
    };
    fetchTransactionsForCard();
  }, [selectedCardId, getTransactions, cards]);

  const filteredTransactions = transactions.filter(transaction => {
    if (!selectedDate) return true;
    const transactionDate = new Date(transaction.transactionDate); return transactionDate.toDateString() === selectedDate.toDateString();
  });
  ...
  const handleExportPDF = () => {
    ...
    transactionsToExport.forEach(item => {
      ...
      const transactionData = [new Date(item.transactionDate).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' }), item.description, `${isDeposit ? '+' : '-'}${formatCurrency(item.amount)}`, isDeposit ? 'Nhan tien' : 'Chuyen/Rut tien', item.status]; tableRows.push(transactionData);
    });
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={vi}>
      ...
      <Box sx={{ minHeight: '100vh', background: `linear-gradient(135deg, ${colors.background} 0%, #e2e8f0 100%)`, p: 3 }}>
        ...
        <Fade in={true} timeout={1000}><Paper elevation={0} sx={{ borderRadius: 4, overflow: 'hidden', background: colors.surface, border: `1px solid ${colors.border}`, minHeight: '500px' }}><Box sx={{ p: 3, background: `linear-gradient(90deg, ${colors.surface} 0%, ${colors.background} 100%)`, borderBottom: `1px solid ${colors.border}` }}><Stack direction="row" justifyContent="space-between" alignItems="center"><Stack direction="row" spacing={2} alignItems="center"><Avatar sx={{ background: colors.gradientSuccess }}><TrendingUpRounded /></Avatar><Box><Typography variant="h6" fontWeight={600} color={colors.textPrimary}>Hoạt động gần đây</Typography><Typography variant="body2" color={colors.textSecondary}>{isFetchingTransactions ? 'Đang tải...' : `${filteredTransactions.length} giao dịch`}</Typography></Box></Stack><Stack direction="row" spacing={2} alignItems="center"><Tooltip title="Xuất danh sách giao dịch ra PDF"><IconButton onClick={handleExportPDF} sx={{ background: colors.gradientWarning, color: 'white', '&:hover': { transform: 'scale(1.1)', background: colors.gradientWarning } }}><PictureAsPdf /></IconButton></Tooltip><DatePicker label="Lọc theo ngày" value={selectedDate} onChange={(newValue) => setSelectedDate(newValue)} slotProps={{ textField: { size: 'small', sx: { '& .MuiOutlinedInput-root': { borderRadius: 2, background: colors.background } } }, inputAdornment: { position: 'end', children: selectedDate && (<IconButton size="small" onClick={() => setSelectedDate(null)} sx={{ marginRight: '-8px' }}><Clear /></IconButton>) } }} sx={{ width: '200px' }} /></Stack></Stack></Box>
                  ...
                  return (
                    <Fade key={item.transactionId || index} in={true} timeout={600 + index * 100}>
                      <Box>
                        ...
                        {/* <<< THÊM MỚI 5: Thêm onClick vào Paper và cursor pointer >>> */}
                        <Paper 
                          elevation={0}
                          ...
                          <CardContent sx={{ p: '20px !important' }}>
                            <Stack direction="row" justifyContent="space-between" alignItems="center">
                              <Stack direction="row" spacing={3} alignItems="center">
                                <Avatar sx={{ width: 48, height: 48, background: isDeposit ? colors.gradientSuccess : colors.gradientWarning, color: 'white' }}>{isDeposit ? <ArrowUpward /> : <ArrowDownward />}</Avatar>
                                  {/* Sửa lại bố cục của phần mô tả */}
                                  <Box> 
                                      <Typography variant="body1" fontWeight={600} color={colors.textPrimary} sx={{ mb: 0.5 }}>
                                          {item.description}
                                      </Typography>
                                      <Chip label={item.status} size="small" sx={{ background: colors.background, color: colors.textSecondary, fontWeight: 500 }} />
                                  </Box>
                              </Stack>
                              <Chip label={`${isDeposit ? '+' : '-'}${formatCurrency(item.amount)}`} sx={{ background: isDeposit ? colors.gradientSuccess : colors.gradientWarning, color: 'white', fontWeight: 700, fontSize: '1rem', height: '36px', borderRadius: 2 }} />
                            </Stack>
                          </CardContent>
                        </Paper>
                      </Box>
      ...
      {/* <<< THÊM MỚI 6: Component Dialog hiển thị chi tiết giao dịch >>> */}
            ...
            <Stack spacing={2}>
                ...
                <Box textAlign="center" my={2}>
                    <Typography 
                        variant="h4" 
                        fontWeight="bold"
                        color={selectedTransaction.transaction_type === 'Deposit' ? 'success.main' : 'error.main'}
                    >
                        {selectedTransaction.transaction_type === 'Deposit' ? '+' : '-'}{formatCurrency(selectedTransaction.amount)} VND
                    </Typography>
                    <Chip 
                        label={selectedTransaction.status} 
                        color={selectedTransaction.status === 'Thành công' ? 'success' : 'warning'}
                        size="small"
                        sx={{ mt: 1 }}
                    />
                </Box>
                ...
                <Stack direction="row" justifyContent="space-between">
                    ...
                    <Typography fontWeight="500">{selectedTransaction.transactionId}</Typography>
                </Stack>
                <Stack direction="row" justifyContent="space-between">
                ...
                <Divider />
                <Stack direction="row" justifyContent="space-between">
                    <Typography color="text.secondary">Tài khoản nguồn</Typography>
                    <Typography fontWeight="500">{selectedTransaction.senderAccount}</Typography>
                </Stack>
                <Stack direction="row" justifyContent="space-between">
                    <Typography color="text.secondary">Tài khoản hưởng</Typography>
                    <Typography fontWeight="500">{selectedTransaction.receiverAccount}</Typography>
                </Stack>
                <Divider />
                <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
                    <Typography color="text.secondary">Nội dung</Typography>
                    <Typography fontWeight="500" textAlign="right" sx={{ maxWidth: '70%' }}>
